...
S5_combined_complete_score.tsv​: A tab-separated file with a header, containing at least 75 columns where columns 3-74 are features and column 75 is the target score for pre-training.  
Peplist_Score​: A tab-separated file without a header, following the same column structure as the main file, and must contain both positive and negative samples for fine-tuning.  

```bash
python MLP_251230_tuneplot.py
```
The script will generate the following output files:  
training_log.txt: A log file recording detailed training progress, including losses for each epoch in both pre-training and fine-tuning phases.  
pretrain_curve.pdf: A plot visualizing the pre-training loss curves (train, validation, and test MSE).  
final_model_tuned.pth: The saved PyTorch model weights after fine-tuning (if fine-tuning is performed).  
scaler.pkl: The fitted StandardScaler object saved via joblib for consistent data preprocessing.  
